...
### График plotly
reg_rf2  %>% 
  select (Регион, Активны) %>% 
  top_n(10, wt = Активны) %>%
  mutate(Регион = fct_reorder(Регион,Активны	)) %>%
  plot_ly(x = ~ Регион, y = ~ Активны) %>% 
  add_column() %>% 
...
### Статистика по заболеваймости на 100 000 населения

```{r}
knitr::kable(left_join(reg_rf2, nas_rf, c("Регион" = "Субъект")) %>% 
transmute(Регион, Заболеваймость = Заболело / Население * 100000,
